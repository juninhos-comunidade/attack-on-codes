...
public class JwtService {
	...
	@PostConstruct
	public void init() {
		Resource privateRes = resourceLoader.getResource("file:" + basePath + "/" + privateKeyName);
		Resource publicRes = resourceLoader.getResource("file:" + basePath + "/" + publicKeyName);
		try {
			this.privateKey = keyUtils.readPrivateKey(privateRes);
			this.publicKey = keyUtils.readPublicKey(publicRes);
			if (keyUtils.isKeySystemGenerated(privateRes) && isTestProfile == false) {
				log.warn("***********************************************************");
				log.warn("SECURITY NOTICE: THE LOADED KEYS WERE GENERATED BY THIS SYSTEM.");
				log.warn("Location: {}", basePath);
				log.warn("Don't use system generated token keys on production");
				log.warn("***********************************************************");
			}
		} catch (Exception e) {
			if (isTestProfile) {
